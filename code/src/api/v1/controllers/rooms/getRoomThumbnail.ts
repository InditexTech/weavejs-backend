...
export const getRoomThumbnailController = () => {
  ...
  return async (req: Request, res: Response): Promise<void> => {
    ...
    if (!(await persistenceHandler.exists(fileName))) {
      res
        .status(404)
        .json({ status: "KO", message: "Room thumbnail doesn't exists" });
      return;
    }
    ...
    if (response && response.readableStreamBody) {
      ...
      try {
        ...
        if (!res.headersSent) {
          res.status(500).json({
            status: "KO",
            message: "Download room thumbnail failed",
          });
        }
      }
    } else {
      res
        .status(500)
        .json({ status: "KO", message: "Error downloading room thumbnail" });
    }
  };
};
