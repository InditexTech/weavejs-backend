...
/**
 * Rate-limits the MCP endpoint to prevent unauthenticated callers from
 * driving unbounded server-side rendering work (CWE-400).
 *
 * Allows at most MAX_REQUESTS per IP within WINDOW_MS, then responds
 * with 429 until the window expires.
 *
...
export function mcpRateLimit(
  ...
  if (existing.count > MAX_REQUESTS) {
    ...
    res
      .status(429)
      .json({ jsonrpc: "2.0", error: { code: -32029, message: "Too many requests. Please try again later." }, id: null });
    return;
  }
